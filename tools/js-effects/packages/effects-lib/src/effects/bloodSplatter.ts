...
const randomInt = (rand: RandomSource, min: number, max: number): number =>
  Math.floor(randomRange(rand, min, max + 1));

const createBlob = (rand: RandomSource, radius: number, points: number, jaggedness: number): number[] => {
  const coords: number[] = [];
...
class BloodSplatterInstance implements EffectInstance<BloodSplatterOptions> {
  readonly id = `blood-splatter-${Math.random().toString(36).slice(2)}`;
  ...
  private updateDroplets(dt: number, rand: RandomSource): void {
  ...
  private addStain(x: number, y: number, rand: RandomSource): void {
    const colors = this.opts.colors ?? [DEFAULT_MID, DEFAULT_DARK];
    const midColor = colors[0] ?? DEFAULT_MID;
    const darkColor = colors[1] ?? DEFAULT_DARK;
...
export const BloodSplatterDefinition: EffectDefinition<BloodSplatterOptions> = {
  ...
  createFromPreset: (position, preset?: EffectPreset, overrides?) => {
    const baseOptions =
      (preset?.options as Partial<BloodSplatterOptions> | undefined) ?? {};
    const merged = {
      ...BloodSplatterDefinition.defaults,
      ...baseOptions,
      ...(overrides as Partial<BloodSplatterOptions> | undefined),
    };
