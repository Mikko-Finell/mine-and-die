...
| Effect patch sequences regress during incremental updates | High       | 🟢 Done  | Repro: kill goblin; state stream rewound sequence and flooded errors; client now clamps incremental regressions and hydrates effects. |
| Effect patches rejected on client | High       | 🟢 Done  | Repro: cast fireball or walk into lava; client now hydrates `effect_pos`/`effect_params` patches so effects update without spam. |
