...
| Effect patches rejected on client                | High       | 🟢 Done  | Client now tracks effect snapshots and instantiates effect views so `effect_pos`/`effect_params` patches apply (cast fireball or walk into lava). |
