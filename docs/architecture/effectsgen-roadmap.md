...
| Scaffold code generator | Parse Go registry, map to TS AST, and emit modules under `client/generated/effects`. | 🟡 In progress | Pipeline now validates inputs, emits a typed effect catalog snapshot to `client/generated/effect-contracts.ts`, and now projects catalog `definition` blocks as `EffectDefinition` literals; contract payload metadata wiring still pending. |
