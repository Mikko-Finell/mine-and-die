...
Plumb the top-level `managedByClient` flag into the effect renderer so scheduling logic uses the authoritative contract metadata instead of nested fallbacks.
