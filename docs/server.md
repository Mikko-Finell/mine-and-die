...
- Fireballs: `triggerFireball` delegates to the projectile template system (`spawnProjectile`). Templates declare speed, range, cooldown, and impact rules while `advanceEffects`/`resolveProjectileImpacts` handle motion, collisions, and any on-hit or on-expiry explosions.
...
- Register new projectile templates in `newProjectileTemplates` to introduce additional ranged archetypes without touching the main advance loop.
