...
func TestMarshalStateSnapshotMarksResync(t *testing.T) {
        hub := newHub()
        hub.advance(time.Now(), 1.0/float64(tickRate))
        ...
        resyncValue, ok := payload["resync"]
        if !ok {
                t.Fatalf("expected resync flag when snapshotting without draining patches")
        }
        resyncBool, ok := resyncValue.(bool)
        if !ok {
                t.Fatalf("expected resync flag to be boolean, got %T", resyncValue)
        }
        if !resyncBool {
                t.Fatalf("expected resync flag to be true when snapshotting without draining patches")
        }
}

func TestResetWorldMarksNextBroadcastAsResync(t *testing.T) {
        hub := newHub()

        hub.advance(time.Now(), 1.0/float64(tickRate))
        if _, _, err := hub.marshalState(nil, nil, nil, nil, nil, true); err != nil {
                t.Fatalf("initial marshalState returned error: %v", err)
        }
        ...
        resyncValue, ok := payload["resync"]
        if !ok {
                t.Fatalf("expected resync flag after reset")
        }
        resyncBool, ok := resyncValue.(bool)
        if !ok {
                t.Fatalf("expected resync flag to be boolean, got %T", resyncValue)
        ...
        if !resyncBool {
                t.Fatalf("expected resync flag to be true after reset")
        }

        // Subsequent broadcasts should clear the resync flag unless another reset occurs.
        data, _, err = hub.marshalState(nil, nil, nil, nil, nil, true)
        if err != nil {
        ...
        if resyncValue, ok := payload["resync"]; ok {
                if resyncBool, ok := resyncValue.(bool); ok && resyncBool {
                        t.Fatalf("expected resync flag to clear after follow-up broadcast")
                }
        }
}
