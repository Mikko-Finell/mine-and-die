		...
		Instance: func(value any) (worldpkg.StatusEffectInstanceConfig, bool) {
			...
			if def.OnTick != nil {
				cfg.Definition.OnTick = func(tickAt time.Time) {
					def.OnTick(worldpkg.StatusEffectTickRuntime{
						Handle: newStatusEffectInstanceHandle(inst, actor),
						Now:    tickAt,
					})
				}
			...
			if def.OnExpire != nil {
				cfg.Definition.OnExpire = func(expireAt time.Time) {
					def.OnExpire(worldpkg.StatusEffectExpireRuntime{
						Handle: newStatusEffectInstanceHandle(inst, actor),
						Now:    expireAt,
					})
				}
			...
			var attachment worldpkg.StatusEffectAttachmentConfig
			includeAttachment := false

			if handle.Attachment.Extend != nil {
				includeAttachment = true
				attachment.Extend = func(expiresAt time.Time) {
					handle.Attachment.Extend(expiresAt)
				}
			}

			attachment.Expire = func(at time.Time) (any, bool) {
				instPtr, _ := handle.Instance.(*statusEffectInstance)
				if instPtr == nil {
					return nil, false
				}
				eff := instPtr.attachedEffect
				if eff == nil {
					return nil, false
				}
