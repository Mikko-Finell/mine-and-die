...
func TestDeterminismHarnessProducesBaseline(t *testing.T) {
	...
	if baseline.Ticks != determinismHarnessTickCount {
		t.Fatalf("expected %d ticks, got %d", determinismHarnessTickCount, baseline.Ticks)
	}
	if baseline.PatchChecksum == "" {
		t.Fatal("expected patch checksum to be populated")
	}
	if baseline.JournalChecksum == "" {
		t.Fatal("expected journal checksum to be populated")
	}
