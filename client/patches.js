...
export function cloneEffectParamsMap(params) {
  const result = {};
...
function createEffectView(entry) {
  if (!entry || typeof entry !== "object") {
    return null;
  }
  ...
  const params = cloneEffectParamsMap(entry.params);
  return {
    id,
...
function cloneEffectView(view) {
  ...
  return {
    ...
    x: toFiniteNumber(view.x, 0),
    y: toFiniteNumber(view.y, 0),
    params: cloneEffectParamsMap(view.params),
  };
}
...
function ensureEffectViewEntry(viewMap, patch) {
  if (!viewMap || typeof viewMap !== "object") {
    return null;
  }
  const id = normalizeEntityId(patch?.entityId);
  if (!id) {
    return null;
  }
  let entry = viewMap[id];
  if (!entry || typeof entry !== "object") {
    entry = { id, x: 0, y: 0, params: {} };
    viewMap[id] = entry;
    return entry;
  }
  entry.id = id;
  if (!entry.params || typeof entry.params !== "object" || entry.params instanceof Map) {
    entry.params = cloneEffectParamsMap(entry.params);
  }
  return entry;
}
...
function applyEffectParams(view, payload) {
  ...
  const source =
    ...
    typeof payload.params === "object"
      ? payload.params
      : payload;
  const params = cloneEffectParamsMap(source);
  view.params = params;
  return { applied: true };
}
...
const PATCH_HANDLERS = {
  ...
  [PATCH_KIND_EFFECT_POS]: {
    target: "effects",
    ensure: ensureEffectViewEntry,
    apply: applyEffectPosition,
  },
  [PATCH_KIND_EFFECT_PARAMS]: {
    target: "effects",
    ensure: ensureEffectViewEntry,
    apply: applyEffectParams,
  },
...
function applyPatchesToSnapshot(baseSnapshot, patches, options = {}) {
  ...
  for (const rawPatch of patches) {
    ...
    if (!view && typeof handlerEntry.ensure === "function") {
      view = handlerEntry.ensure(viewMap, patch) || null;
    }
    if (!view) {
      errors.push(
        ...
        for (const rawPatch of patchList) {
          ...
          let entityKnown =
            viewMap && typeof viewMap === "object"
              ...
              : false;
          if (!entityKnown && typeof handlerEntry.ensure === "function") {
            entityKnown = true;
          }
          if (entityKnown) {
            replayablePatches.push(rawPatch);
